...
const (
	msgNoEventYet  = "(no event yet)"
	msgUnknownExpr = "(unknown expression)"
)

// Evaluate returns the value of expr in the context of ev (e.g. pid, tgid, cpu).
// Normalizes expr (lowercase, trim). Returns msgNoEventYet if ev is nil,
// msgUnknownExpr for unsupported expr.
//
//nolint:gocyclo // expression switch is intentionally flat for readability
func Evaluate(ev *runtime.Event, expr string) string {
	expr = strings.ToLower(strings.TrimSpace(expr))
	if ev == nil {
		return msgNoEventYet
	}
	switch expr {
	case "pid":
...
func ConditionPasses(ev *runtime.Event, cond string) bool {
	...
	if cond == "1" || strings.EqualFold(cond, "true") || strings.EqualFold(cond, "yes") {
		return true
	}
	if cond == "0" || strings.EqualFold(cond, "false") || strings.EqualFold(cond, "no") {
		return false
	}
	...
	if val == msgNoEventYet || val == msgUnknownExpr {
		return false
	}
