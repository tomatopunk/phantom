...
**Hardened / CI hosts:** [`scripts/e2e_linux_bpf_env.sh`](../scripts/e2e_linux_bpf_env.sh) raises **memlock** where possible and applies **`setcap`** (**cap_sys_resource**, **cap_bpf**) when **sudo** is available. On **GitHub Actions**, the shell e2e scripts start the agent with **`sudo -n -E`** via **`phantom_e2e_run_agent_sudo`**, which runs **`bash -c 'ulimit -l unlimited; exec …'`** before **`exec`** so the process does not inherit an overly low **RLIMIT_MEMLOCK** soft limit from the runner. **Go** e2e on Linux uses **`bash` + `ulimit` + `exec`** when starting the agent **without** sudo (so **`exec`** replaces **`bash`** and teardown still targets the agent PID). The **`E2E_AGENT_USE_SUDO=1`** path uses **`sudo -n -E`** directly (root should satisfy **`RemoveMemlock()`**). **`make test-e2e-mr`** **`setcap`**s **`phantom-agent`** in CI so **Go** e2e can run the binary **without** sudo while still satisfying **cilium/ebpf** memlock expectations.
